 ...
 * @version V1.0
 *********************************************************/
public class DownloadUpgradeHelper {
    ...
    private int dialog_type_wifi = 1;
    private int dialog_type_update = 2;
    
    private android.app.Dialog dialog;
    
    public DownloadUpgradeHelper(android.content.Context context){
    ...
    public synchronized boolean startDownload(boolean type){
		...
		if (type && !UpgradeUtils.isWifiActive(context)) {
			UpgradeUtils.showWifiPromptDialog(context, dialog_type_wifi, mDialogClickListener);
			return true;
		}
	...
	@SuppressLint("NewApi")
	public boolean addDownload(String filename, String url){
...
//	        	request.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED);
    ...
    public synchronized void isUpdate(final boolean type){
    	new Thread(){
    		@Override
    		public void run() {
				try {
    ...
    private DialogClickListener mDialogClickListener  = new DialogClickListener() {
		...
		public void onPositiveClick(int flag, Object object, android.content.DialogInterface dialog) {
			...
			if (flag == dialog_type_update) {
				...
				addDownload(verion.getObject().toString(), verion.getDownloadUrl());
			} else if (flag == dialog_type_wifi) {
				isUpdate(true);
			}
		}

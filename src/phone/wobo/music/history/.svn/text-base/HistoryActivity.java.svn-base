...
public class HistoryActivity extends BaseActivity implements
		...
		adapter = new FavoritesAdapter(mContext) {
			protected void play(View view, int position) {
				...
				if (isExistMusic(listMusic)) {
					FuncUtils.beginClickAnimation(HistoryActivity.this, view,
			...
			protected void add2download(View view, int position) {
				...
				if (isExistMusic(listMusic)) {
					FuncUtils.beginClickAnimation(HistoryActivity.this, view,
							phone.wobo.music.R.drawable.btn_download);
	...
	private void deleteHistory() {
		if (currentCheckNumber > 0) {
			...
			new WoboDialog(this, "移除播放记录", "确定要移除你所选择的歌曲吗？") {
				@Override
				...
				public void setOnPositiveListener() {
					...
					for (int i = 0; i < deleteList.size(); i++) {
						if (deleteList.get(i).isSelected()) {
							handleHistory(deleteList.get(i));
						}
					}
					cancelEditor();
					loadData();
	...
	// 处理收藏
	private void handleHistory(MusicInfo info) {
		Log.d("info", info.getName());
		try {
			if (dbHelper.isExistHistory(info)) {
				dbHelper.deleteHistory(info);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}

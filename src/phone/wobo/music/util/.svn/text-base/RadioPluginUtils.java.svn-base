 ...
 * 插件工具类
 */
public class RadioPluginUtils {
	private final String JAR_NAME = "radio.wobo.plugin.jar";
	...
	private void saveVersion(String version) {
		...
		Editor edit = p.edit();
		edit.putString("radioJarVersion", version);
		edit.commit();
	}
	...
	private String getJarVersion4Local() {
		...
		String version = p.getString("radioJarVersion", "");

		return version;
	}

	private String getJarVersion4Server() {
		String url = Config.PLUGIN_DOMAIN + "plugin/radio/addon.xml";
		try {
	...
	private String getJarPath() {
		String rootPath = TVBoxApplication.getAppContext().getFilesDir()
				.getAbsolutePath();
		return rootPath + "/plugin/radio.wobo.plugin/" + JAR_NAME;
	}

	private String getTempJarPath() {
		String rootPath = TVBoxApplication.getAppContext().getFilesDir()
				.getAbsolutePath();
		return rootPath + "/plugin/radio.wobo.plugin/temp/" + JAR_NAME;
	}
}

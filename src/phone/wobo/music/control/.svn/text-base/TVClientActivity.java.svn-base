...
public class TVClientActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mCtc = null;
		mCds = new CommDatagramSocket(this, mInitIpString, mPortUdp, mHandler);
		mCds.receive();
		initView();
		registerReceiver();		
...
//		startInteract();
	...
	protected void DealReceiveBytes(byte[] receive) {
		...
		case Protocol.input_method_show:
			...
			if (length == 0) {
				...
				Intent inputIntent = new Intent(TVClientActivity.this,
						InputControl.class);
			...
			startActivity(inputIntent);
			break;
		case Protocol.version_validate:     
            Log.d(TAG,"Received Protocol.version_validate");  
            this.startInteract();
            /*int verLength = receive.length - 1;         
            if(verLength <= 0){
                this.startInteract();
                break;
            }           
            ...
            String ServerVerName = new String(receive, 1, verLength);           
            if(pinfo.versionName.equals(ServerVerName)){
                this.startInteract();
                break;
            } */          
            /*AlertDialog.Builder updateDlg = new AlertDialog.Builder(this);
            updateDlg.setTitle(R.string.dialog_title_version_update);
            updateDlg.setMessage(R.string.dialog_msg_new_version);
            updateDlg.setPositiveButton(R.string.dialog_pos_btn, 
                    this.new UpdateDialogPosBtnListener(ServerVerName));
            updateDlg.show();*/           
            break;
		default:
			break;
	...
	protected void DealReceiveString(String textString) {
		...
		if ("cmd___online".equals(textString)) {
		    ...
		    FuncUtils.makeText(TVClientActivity.this, textString );
		} else if ("cmd___onWindowShown".equals(textString)) {
			Intent inputIntent = new Intent(TVClientActivity.this,
					InputControl.class);
			startActivity(inputIntent);
		} else if ("cmd___finishInputView".equals(textString)) {
		    FuncUtils.makeText(TVClientActivity.this, "cmd___finishInputView" );
		}
	}
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals("cn.abel.action.connect")) {
				...
				case WifiManager.WIFI_STATE_DISABLED:
				    FuncUtils.makeText(TVClientActivity.this, R.string.wifi_disable );
					TVClientActivity.this.mIsConnect
							.setText(TVClientActivity.this.getResources()
									.getString(R.string.wifi_disable));
					break;
